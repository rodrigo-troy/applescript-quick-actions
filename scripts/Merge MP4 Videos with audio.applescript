on run {input, parameters}
	-- Merge MP4 Videos using FFmpeg (sorted by creation date, no audio, interpolated)
	...
	tell application "Finder"
		...
		repeat with f in mp4Files
			set pPath to POSIX path of (f as alias)
			set epochTime to do shell script "stat -f '%B' " & quoted form of pPath
			set end of sortableList to epochTime & "|||" & pPath
		end repeat

		set oldDelims to AppleScript's text item delimiters
		set AppleScript's text item delimiters to linefeed
		set rawString to sortableList as string
		set AppleScript's text item delimiters to oldDelims

		set sortedString to do shell script "echo " & quoted form of rawString & " | sort -n"

		set sortedList to paragraphs of sortedString
		repeat with itemEntry in sortedList
			if length of itemEntry > 0 then
				set oldDelims to AppleScript's text item delimiters
				set AppleScript's text item delimiters to "|||"
				set filePath to text item 2 of itemEntry
				set AppleScript's text item delimiters to oldDelims

				set escapedPath to my replaceText(filePath, "'", "'\\''")
