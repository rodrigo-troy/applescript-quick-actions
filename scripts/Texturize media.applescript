on run {input, parameters}
	-- Apply Pixelmator Pro's texture adjustment at a user-chosen intensity (-100 to 100, default 20)
	-- to each selected image or MP4 video. Saves a new file alongside the original with a "-textured"
	...
	try
		set userResponse to display dialog "Enter texture intensity (-100 to 100):" default answer "20" buttons {"Cancel", "OK"} default button "OK" with icon note
	on error number -128
		return
	end try
	try
		set textureValue to (text returned of userResponse) as number
	on error
		display dialog "Invalid number." buttons {"OK"} default button 1 with icon stop
		return
	end try
	if textureValue < -100 or textureValue > 100 then
	...
	display notification "Adding texture to " & totalFiles & " file(s)..." with title "Texture" sound name "default"
	...
	repeat with imgRef in imageFiles
		...
		set outputPath to parentDir & "/" & nameNoExt & "-textured." & inExt
		...
		try
			tell application "Pixelmator Pro"
				...
				tell the color adjustments of the first layer of the front document
					set its texture to textureValue
				end tell
			...
			display notification "Textured " & (processedCount as string) & " of " & (totalFiles as string) with title "Texture"
		on error errMsg
	...
	if errorCount is 0 then
		display alert "Texture Complete" message (processedCount as string) & " file(s) textured."
	else
		display alert "Texture Complete" message (processedCount as string) & " processed, " & (errorCount as string) & " error(s)." & return & return & errorMessages as warning
	end if
